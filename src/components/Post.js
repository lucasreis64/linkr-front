...
import { getComments, postLike, removeLike } from "../service/api";
...
export default function Post(props) {
    ...
    const [isLiked, setIsLiked] = useState(
        [...data?.likes_users]?.includes(user.username) ||
            [...data?.likes_users]?.includes(user.id)
    );
    ...
    const [message, setMessage] = useState(
        likesCount > 0 ? updateLikeTooltip() : ""
    );
    ...
    useEffect(
        () => {
            getComments(token.token, data.id)
                .then((res) => {
                    setComment(res.data);
                })
    ...
    function updateLikeTooltip() {
        ...
        if (isLiked) {
            ...
            if (likesCount > 1) {
                param2 = ([...data?.likes_users]?.filter(
                    (i) => i !== user.username
                ))[0];
            }
        }
            ...
            <PostContainer>
                    ...
                    <div className="like-actions">
                        {isLiked ? (
                            <FaHeart
                                cursor={"pointer"}
                                color="red"
                                onClick={() => tapLike("dislike")}
                            />
                        ) : (
                            <FaRegHeart
                                cursor={"pointer"}
                                onClick={() => tapLike("like")}
                            />
                        )}
                        <div>
                            {likesCount > 0 ? (
                ...
                <div className="right">
                    <div className="header">
                        ...
                        <div
                            className={`menu-op ${isEditable ? "" : "hidden"}`}
                        >
                            <FaPen
                                cursor={"pointer"}
                    ...
                    >
                        <div className="link-container">
                            ...
                            <Image url={data.link_metadata?.image} />
                        </div>
                    </a>
                </div>
            </PostContainer>
...
const B2 = styled.div`
    ...
    margin: 10px;
`;
const PostAndCommentContainer = styled.div`
    margin-bottom: 16px;
`;
